...
- 先把“点击查看详情”做成一个稳定可扩展的入口
- 再决定后续优先往“更强的文件信息”还是“更强的音频元数据”推进

因此，详情面板现在可以视为已经进入第一阶段可用状态，后续工作重点会转向：
...
这一轮没有继续扩功能，而是把两类容易持续制造混乱的产品细节先收口：
...
- 原始 Android `content://` 对用户几乎没有价值
- 但它又是当前平台层稳定工作的真实标识，不能为了“好看”直接替换掉底层存储值
- 因此最稳妥的方式就是：底层保真，展示层格式化

当前这一层已经足够支撑第一版继续推进；如果后续再做：
